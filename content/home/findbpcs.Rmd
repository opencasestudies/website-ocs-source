...
Packages_again <- pmap(list(OCS_info$packages), make_string) %>% unlist()
     Obj_again <- pmap(list(OCS_info$objectives), make_string) %>% unlist()
Questions_again <- pmap(list(OCS_info$questions), make_string) %>% unlist()
...
OCS_info_table <- OCS_info %>% 
  select(`Case Study`, `GitHub Repository`, Packages, Objectives)%>%
  DT::datatable(
    rownames = FALSE,
...
widgetframe::frameWidget(OCS_info_table, width = "100%", height = "100%" )
```

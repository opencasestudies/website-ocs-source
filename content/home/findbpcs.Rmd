...
get_objectives <- function(URL){
  xml2::read_html(URL) %>%
        ...
        filter(!str_detect(value, "\\?")) %>%
        mutate(value = str_remove(value, "\n")) %>%
        filter(!str_detect(value, "Donohue|Lott|Census|Kristen|themagick|compositionsfor|Bureau|A |guide|discussion|Greatvideo|More examples|consciousness|erratic|bluish|limp|snore|clammy|purplish|modifythese|acrossyears|unable to talk|Anexplanation|Alist|cheatsheets|Acourse|http"))
}
```
